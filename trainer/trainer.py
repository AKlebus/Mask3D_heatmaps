...
class InstanceSegmentation(pl.LightningModule):
    ...
    def export(self, pred_masks, scores, pred_classes, file_names, decoder_id, heatmaps = None):
        ...
        pred_heatmap_path = f"{base_path}/heatmap"

        Path(pred_mask_path).mkdir(parents=True, exist_ok=True)
        Path(pred_heatmap_path).mkdir(parents=True, exist_ok=True)
        ...
        with open(f"{base_path}/{file_name}.txt", "w") as fout:
            ...
            for instance_id in range(len(pred_classes)):
                ...
                if score > self.config.general.export_threshold:
                    ...
                    if heatmaps is not None:
                        np.savetxt(f"{pred_heatmap_path}/{file_name}_{real_id}.txt", heatmap, fmt="%f")
